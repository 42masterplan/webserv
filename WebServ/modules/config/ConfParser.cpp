...
/**
 * @brief ListenPort들을 반환하는 함수
 *
 * @return const std::vector<int>& listen 포트들을 담고있는 vector
 */
const std::vector<int>& ConfParser::getListenPorts()const{
	return(http_store_[0].getListenPorts());
}
...
void	ConfParser::makeBlock(std::string line, std::ifstream& input, int &line_len){
	...
	switch(checkBlockName(block_name)){
		case HTTPBLOCK : makeHttpBlock(input);
			break;
		case SERVERBLOCK : throw(std::runtime_error("You need to create an HTTP block first.(server)"));
		case LOCATIONBLOCK : throw(std::runtime_error("You need to create an HTTP block first.(location)"));
