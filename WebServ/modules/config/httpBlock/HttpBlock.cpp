 ...
 */
const std::vector<int>& HttpBlock::getListenPorts()const{
	return(port_store_);
}
...
void	HttpBlock::refineAll(){
	...
	for (size_t i = 0; i < serv_store_.size(); i++){
		...
		port_store_.push_back(serv_store_[i].getListen());
	}
	std::sort(port_store_.begin(), port_store_.end());
	int bef = -1;
	for (size_t i = 0; i < port_store_.size(); i++){
		if (port_store_[i] == bef)
...
void	HttpBlock::makeBlock(std::string line, std::ifstream& input, int& line_len_){
	...
	switch(checkBlockName(block_name)){
		...
		case SERVERBLOCK : makeServerBlock(input, line_len_);
			break;
